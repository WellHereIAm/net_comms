...
fn main() {
    ...
    let receivers: Vec<String> = long_text.to_string().split_ascii_whitespace().map(|x| {x.to_string()}). collect();
    let command = Command::Send(MessageKind::Text, 1, receivers, "hello how are you".to_string().to_buff(), None);
